var Player = function(){
	...
	this.animate = function() {
		...
		if (MOUSE.clic){
			...
			if(game.map.cells[pos.y][pos.x].type != 'Bloc')
	...
	this.isCollideSomething = function() {
		...
		if (posY.x >= 0 && posY.y < game.map.cells.length && posX.x >= 0 && posX.y < game.map.cells.length ) {
			if(game.map.cells[posY.y][posY.x].type == "Coin")
				return this.getCoin();
			if (game.map.cells[posY.y][posY.x].type == "Bloc") {
				hit = true;
			}else if (game.map.cells[posX.y][posX.x].type == "Bloc") {
				hit = true;
			}
		}
