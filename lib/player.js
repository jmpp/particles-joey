var Player = function(){
	...
	this.animate = function() {
		...
		for(var i in this.p)
		{
			...
			if      (this.p[i].sx >= 0) this.p[i].sx = this.p[i].sx >  this.r/2 ?  this.r/2 : this.p[i].sx;
			else if (this.p[i].sx <  0) this.p[i].sx = this.p[i].sx < -this.r/2 ? -this.r/2 : this.p[i].sx;
			if      (this.p[i].sy >= 0) this.p[i].sy = this.p[i].sy >  this.r/2 ?  this.r/2 : this.p[i].sy;
			else if (this.p[i].sy <  0) this.p[i].sy = this.p[i].sy < -this.r/2 ? -this.r/2 : this.p[i].sy;
		}
	...
	this.render = function() {
		...
		Rect({x:this.x, y:this.y, w:this.r, h:this.r, c:'#000'});
		// Particles
		for(var i in this.p)
	...
	this.teleport = function() {
		if (this.particlesDisappearing) {
			...
			if (nbZeroParticles >= this.p.length) {
				...
				this.x = MOUSE.lastPosX - this.r;
				this.y = MOUSE.lastPosY - this.r;
	...
	this.notifyMap = function() {
		if(game.map.cells[this.cell.y] && game.map.cells[this.cell.y][this.cell.x]){
			...
			this.cell.x = Math.floor(this.x / game.map.sizeCell);
			this.cell.y = Math.floor(this.y / game.map.sizeCell);
			if(game.map.cells[this.cell.y] && game.map.cells[this.cell.y][this.cell.x]){
